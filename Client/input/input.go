...
func MovementKeys(Window *glfw.Window, player *utils.Player) {
	if Window.GetKey(glfw.KeyW) == glfw.Press {
		player.SetVelocity(utils.NewVector2(player.GetVelocity().X, 1))
	}
	if Window.GetKey(glfw.KeyW) == glfw.Release && player.GetVelocity().Y > 0 {
		player.SetVelocity(utils.NewVector2(player.GetVelocity().X, 0))
	}
	if Window.GetKey(glfw.KeyA) == glfw.Press {
		player.SetVelocity(utils.NewVector2(-1, player.GetVelocity().Y))
	}
	if Window.GetKey(glfw.KeyA) == glfw.Release && player.GetVelocity().X < 0 {
		player.SetVelocity(utils.NewVector2(0, player.GetVelocity().Y))
	}
